# -*- coding: utf-8 -*-
"""部署 10s 快速重连版 seeinps 到测试服务器(PM 仅注释改动, 不重启)"""
import time
...
try:
    ...
    sftp.close()
    show("上传+备份+替换", *sudo_run(ssh, "cp /root/seeinp/seeinps /root/seeinp/seeinps.bak.0801b && cp /tmp/seeinps.new /root/seeinp/seeinps && chmod +x /root/seeinp/seeinps && md5sum /tmp/seeinps.new /root/seeinp/seeinps | awk '{print $1}' | uniq -c"))
    show("停止旧进程", *sudo_run(ssh, "pkill -f './seeinps'; sleep 1; ps aux | grep seeinps | grep -v grep || echo '已停止'"))
