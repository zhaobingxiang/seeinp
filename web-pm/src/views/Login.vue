  ...
  <div class="login-page">
    <div class="login-card">
      ...
      <el-form :model="form" :rules="rules" ref="formRef">
        ...
        <el-form-item>
          <el-button type="primary" size="large" style="width: 100%" :loading="loading" :disabled="lockRemaining > 0" @click="handleLogin">登录</el-button>
        </el-form-item>
      </el-form>
      <div class="footer">
...
const handleLogin = async () => { if (!formRef.value) return; await formRef.value.validate(async (valid) => { if (!valid || loading.value) return; if (needCaptcha.value && !captchaText.value) { ElMessage.warning("请输入验证码"); return }; loading.value = true; try { const payload: any = { ...form, captcha_id: needCaptcha.value ? captchaId.value : "", captcha_text: needCaptcha.value ? captchaText.value : "" }; const res: any = await authApi.login(payload); if (res.code === 0) { localStorage.setItem("pm_token", res.data.access_token); ElMessage.success("登录成功"); router.push("/pm/dashboard") } else handleLoginFailure(res.code, res.data) } catch (e: any) { const d = e.response?.data; if (d && typeof d.code === "number") handleLoginFailure(d.code, d.data); else ElMessage.error(d?.message || "登录失败") } finally { loading.value = false } }) }
