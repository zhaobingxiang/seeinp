  ...
  <Layout>
    <div class="page-card">
      ...
      <div style="padding:12px 20px 20px">
        <el-table :data="users" v-loading="loading">
          ...
          <el-table-column label="账号" width="90"><template #default="{row}"><el-tag :type="row.status === 1 ? 'success' : 'danger'" size="small">{{ row.status === 1 ? '启用' : '禁用' }}</el-tag></template></el-table-column>
          <el-table-column label="在线" width="90"><template #default="{row}"><el-tag :type="row.online ? 'success' : 'info'" size="small">{{ row.online ? '在线' : '离线' }}</el-tag></template></el-table-column>
    ...
    <el-dialog v-model="showCreate" title="创建用户" width="450px" @closed="resetCreate">
      <el-form :model="createForm" :rules="createRules" ref="createFormRef" label-width="80px">
        <el-form-item label="用户名" prop="username"><el-input v-model="createForm.username" placeholder="至少 3 个字符" /></el-form-item>
